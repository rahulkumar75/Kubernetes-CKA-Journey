...
### ❌ "The private certificate is sent to the browser"

Incorrect.

```text
Server → Certificate
Server → keeps Private Key
```

---

### ❌ "Base public/private keys automatically provide authentication"

Not by themselves.

The client needs a trusted mechanism to associate the public key with the intended identity.

---

### ❌ "CSR is the certificate"

No.

```text
CSR → Request
Certificate → CA-issued identity binding
```

---

### ❌ "Base64/encryption protects the certificate"

Base64 is only an encoding.

---

### ❌ "HTTPS uses asymmetric encryption for all traffic"

Modern TLS normally uses asymmetric cryptography during authentication/key establishment and symmetric cryptography for bulk data encryption.
...
CSR
    ↓
Contains public key + identity/request information
    ↓
Sent to CA
    ↓
CA validates
...
* **CA → validates and signs certificates**
* **Certificate → identity + public key + CA signature**
* **TLS → authentication + secure session**
* **HTTPS → HTTP over TLS**
* **`.key` → private key**
